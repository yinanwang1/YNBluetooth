...
- (void)dismiss {
    ...
    [UIView animateWithDuration:0.15
                     ...
                     animations:^{
                         weakSelf.hudView.transform = CGAffineTransformScale(self.hudView.transform, 0.8f, 0.8f);
                         if(weakSelf.isClear) // handle iOS 7 UIToolbar not answer well to hierarchy opacity change
                             weakSelf.hudView.alpha = 0.0f;
                         else
                             weakSelf.alpha = 0.0f;
                     }
                     completion:^(BOOL finished) {
                         if(weakSelf.alpha == 0.0f || weakSelf.hudView.alpha == 0.0f) {
                             weakSelf.alpha = 0.0f;
                             weakSelf.hudView.alpha = 0.0f;
                             ...
                             [weakSelf.indefiniteAnimatedView removeFromSuperview];
                             weakSelf.indefiniteAnimatedView = nil;
