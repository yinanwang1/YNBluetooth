...
- (void)viewDidLoad {
    ...
    [self createUI];
    //初始化数据
    self.sect = [NSMutableArray arrayWithObjects:@"read value",@"write value",@"desc",@"properties", nil];
    self.readValueArray = [[NSMutableArray alloc]init];
    self.descriptors = [[NSMutableArray alloc]init];
    //配置ble委托
    [self babyDelegate];
    //读取服务
    self.baby.channel(channelOnCharacteristicView).characteristicDetails(self.currPeripheral,self.characteristic);
...
//            NSLog(@"CharacteristicViewController CBDescriptor name is :%@",d.UUID);
    ...
    [self.baby setBlockOnReadValueForDescriptorsAtChannel:channelOnCharacteristicView block:^(CBPeripheral *peripheral, CBDescriptor *descriptor, NSError *error) {
        for (int i = 0; i < weakSelf.descriptors.count; i++) {
            if (weakSelf.descriptors[i] == descriptor) {
...
//                NSString *valueStr = [[NSString alloc]initWithData:descriptor.value encoding:NSUTF8StringEncoding];
    ...
    //设置通知状态改变的block
    [self.baby setBlockOnDidUpdateNotificationStateForCharacteristicAtChannel:channelOnCharacteristicView block:^(CBPeripheral *peripheral, CBCharacteristic *characteristic, NSError *error) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (indexPath.section) {
        case 0:
            //read value
        {
            cell.textLabel.text = [self.readValueArray objectAtIndex:indexPath.row];
...
//            cell.detailTextLabel.text = [readValueArray valueForKey:@"stamp"];
        ...
        //desc
        {
            CBDescriptor *descriptor = [self.descriptors objectAtIndex:indexPath.row];
...
-(UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    switch (section) {
            ...
            //write value
        {
            ...
            title.text = [self.sect objectAtIndex:section];
