...
@interface BabyPeripheralManager ()

@property (nonatomic, assign) int PERIPHERAL_MANAGER_INIT_WAIT_TIMES;
@property (nonatomic, assign) int didAddServices;
...
- (BabyPeripheralManager *(^)(NSArray *array))addServices {
    return ^BabyPeripheralManager*(NSArray *array) {
        self.services = [NSMutableArray arrayWithArray:array];
...
- (void)addServicesToPeripheral {
    if ([self isPoweredOn]) {
        ...
        self.addServiceTask = [NSTimer scheduledTimerWithTimeInterval:3 target:self selector:@selector(addServicesToPeripheral) userInfo:nil repeats:NO];
    }
}
